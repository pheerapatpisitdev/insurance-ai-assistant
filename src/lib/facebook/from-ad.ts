 ...
 *
 * Meta hands three things over, and they are read in the order of how much they can be
 * trusted:
 *
 *  1. `ref` — what the m.me link called itself. The agency writes it, so it says exactly what
 *     was meant: m.me/<page>?ref=lifeprotect.
 ...
 *  3. `ad_id` — a number that means nothing by itself, looked up in the advertisement figures
 *     the system already syncs, where the advertisement has the name its author gave it.
 *
 * Nothing here is configured anywhere. An agency that names its advertisements after the plan
 * they sell — which is what naming an advertisement is for — is understood without being
 * asked to keep a second list in step with the first.
 */
